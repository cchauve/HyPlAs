...
//    gzFile plasmid_out = gzopen(argv[4], "w");
    ...
    FILE *unknown_neither_out = popen((string{"gzip - > "} + string{argv[6]}).c_str(), "w");
    FILE *unknown_both_out = popen((string{"gzip - > "} + string{argv[7]}).c_str(), "w");
    FILE *unmapped_out = popen((string{"gzip - > "} + string{argv[8]}).c_str(), "w");
